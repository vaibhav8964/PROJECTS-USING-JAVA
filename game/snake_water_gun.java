import java.util.Random;
import java.util.Scanner;

public class snake_water_gun
{
        Scanner sc = new Scanner(System.in);
        Random rand = new Random();
        int comp_inp, user_inp, input;
        void random_no()
        {
            comp_inp = 1 + rand.nextInt(3);
        }
        int intro()
        {
            System.out.println("                                WELCOME TO ROCK PAPER SCISSOR GAME");
            System.out.println("                               ************************************");
            System.out.println("                        MENU");
            System.out.println("                       ------ ");
            System.out.println("                    1. TUTORIAL");
            System.out.println("                    2. NEW GAME\n");
            System.out.print("        ENTER YOUR OPTION(1/2) :- ");
            input = sc.nextInt();
            return input;
        }
        void tutorial()
        {
            System.out.println("\n                                        HOW TO PLAY");
            System.out.println("                                       *************");
            System.out.println("                   -> JUST TYPE THE NUMBER CORRESPONDING TO THE SYMBOL YOU WANT TO ENTER.");
            System.out.println("                   -> FOR EXAMPLE :     ");
            System.out.println("                                   THE AVAILABLE OPTIONS ARE:- ");
            System.out.println("                                        1.SNAKE");
            System.out.println("                                        2.GUN");
            System.out.println("                                        3.WATER");
            System.out.println("                      ->IF YOU WANT TO CHOOSE SNAKE AS YOUR OPTION THAN JUST CHOOSE 1 AS YOUR INPUT");
            System.out.println("                    ->IF YOU WANT TO CHOOSE GUN AS YOUR OPTION THAN JUST CHOOSE 2 AS YOUR INPUT");
            System.out.println("                    ->IF YOU WANT TO CHOOSE WATER AS YOUR OPTION THAN JUST CHOOSE 3 AS YOUR INPUT");
            System.out.println("     NOTE:-        WATER DESTROY GUN  ");
            System.out.println("                   SNAKE VENAMIZE WATER  ");
            System.out.println("                   GUN KILLS SNAKE  ");
            System.out.println("             TAKING TO NEW GAME WINDOW ");
            try
            {
                Thread.sleep(10000);
            }
            catch (Exception e)
            {        }
        }

        void userinput()
        {
            System.out.println("\n                     NEW GAME");
            System.out.println("                    **********");
            System.out.println("        PLEASE CHOOSE YOUR SYMBOL FROM THE OPTIONS BELOW.......");
            System.out.println("                   THE AVAILABLE OPTIONS ARE:- ");
            System.out.println("                      1.SNAKE");
            System.out.println("                      2.GUN");
            System.out.println("                      3.WATER");
            System.out.print("        ENTER YOUR OPTION(1/2/3) :- ");
            user_inp = sc.nextInt();
            switch (user_inp)
            {
                case 1:
                    System.out.println("USER : SNAKE");
                    break;
                case 2:
                    System.out.println("USER : GUN");
                    break;
                case 3:
                    System.out.println("USER : WATER");
                    break;
            }
            switch (comp_inp)
            {
                case 1:
                    System.out.println("COMPUTER : GUN");
                    break;
                case 2:
                    System.out.println("COMPUTER : WATER ");
                    break;
                case 3:
                    System.out.println("COMPUTER : SNAKE");
                    break;
            }
        }
        void condition()
        {
            switch(comp_inp)
            {
                case 1 :  if(user_inp==1)
                {
                    System.out.println("COMPUTER WON");
                }
                else if(user_inp==2)
                {
                    System.out.println("ITS A DROW");
                }
                else if(user_inp==3)
                {
                    System.out.println("USER WON");
                }
                    break;
                case 2 :  if(user_inp==2)
                {
                    System.out.println("COMPUTER WON");
                }
                else if(user_inp==3)
                {
                    System.out.println("ITS A DROW");
                }
                else if(user_inp==1)
                {
                    System.out.println("USER WON");
                }
                    break;
                case 3 :  if(user_inp==3)
                {
                    System.out.println("COMPUTER WON");
                }
                else if(user_inp==1)
                {
                    System.out.println("ITS A DROW");
                }
                else if(user_inp==2)
                {
                    System.out.println("USER WON");
                }
                    break;
            }
        }
    }






